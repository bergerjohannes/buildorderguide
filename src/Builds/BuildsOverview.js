...
const BuildsOverview = (props) => {
  ...
  if (searchQuery !== null) {
    buildsToDisplay = builds.filter(
      (build) =>
        build.title.toUpperCase().indexOf(searchQuery.trim().toUpperCase()) !==
          -1 ||
        (build.civilization !== Constants.Civ.Generic &&
          build.civilization
            .toUpperCase()
        ...
        build.author.toUpperCase().indexOf(searchQuery.trim().toUpperCase()) !==
          -1 ||
        `${build.pop.feudalAge}`.indexOf(searchQuery.trim().toUpperCase()) !==
          -1
    );
  }
  if (civilization !== null) {
    buildsToDisplay = buildsToDisplay.filter(
      (build) =>
        civilization === Constants.Civ.Generic ||
        build.civilization
          .toUpperCase()
          .indexOf(civilization.trim().toUpperCase()) !== -1
    );
  }
      ...
      (build) =>
        ...
        build.attributes.filter(
          (attribute) =>
            attribute
              .toUpperCase()
              .indexOf(buildOrderType.trim().toUpperCase()) !== -1
        ).length > 0
    );
  }

  if (sorting === Constants.Sorting.ByRating) {
    buildsToDisplay = _.orderBy(
      buildsToDisplay,
      ["rating", "title"],
      ["desc", "asc"]
    );
  ...
  if (
    sorting === Constants.Sorting.Alphabetically ||
    sorting === Constants.Sorting.FavoritesOnly
  ) {
    buildsToDisplay = _.orderBy(buildsToDisplay, ["title"], ["asc"]);
  }

  if (sorting === Constants.Sorting.FavoritesOnly) {
    buildsToDisplay = buildsToDisplay.filter((build) =>
      favorites.some((entry) => entry === build.id)
    );
  }
  ...
  return (
    <div>
      ...
      {builds.length === 0 && (
        <LoadingIndicator text={"Loading build orders .."} />
      )}
      {builds.length > 0 && (
        <div class="w-full flex justify-center mb-5">
          <FilterView
            ...
            setBuildOrderType={setBuildOrderType}
      ...
      {builds.length > 0 && (
        <div class="w-11/12 md:w-1/2 lg:1/2 xl:w-1/3 m-auto">
          <Input
      ...
      <div class="w-11/12 md:w-10/12 lg:w-9/12 mx-auto mb-10 flex flex-wrap justify-center">
        {buildsToDisplay !== undefined &&
          buildsToDisplay.map((build) => (
            <BuildPreviewCard
              build={build}
              fav={favorites.some((entry) => entry === build.id)}
              favBuildWithId={favBuildWithId}
            />
          ))}
        {buildsToDisplay !== undefined && buildsToDisplay.length === 0 && (
