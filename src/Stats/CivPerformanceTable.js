...
const CivPerformanceTable = (props) => {
    return (
        <div class='text-main-dark'>
            <div class='flex'>
                <p class='w-1/2 text-left ml-2 md:ml-8'><strong>Civ</strong></p>
            ...
            {props.data.map((stat, index) => (
                <div class='flex text-center space-y-2'>
                    <div class='flex w-1/2 ml-2 md:ml-8'><img class='w-5 h-5 mr-2' src={require('../Images/Civilizations/' + CivInfoService.getCivilizationNameForIndex(index) + '.png')} alt={CivInfoService.getCivilizationNameForIndex(index)} /><p>{CivInfoService.getCivilizationNameForIndex(index)}</p></div>
