...
exports.recalculateAggregatedRatingsOnPublish = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        const publishedBuildRef = db.collection('published-builds').doc(context.params.docId);

        if (change.before.data() === undefined) {
            const buildRatingQuery = db.collection('ratings').where('build_id', '==', context.params.docId);
            ...
            return publishedBuildRef.update({ avg_rating: avgRating, number_of_ratings: numberOfRatings });
        }
    })
    ...
    .onWrite(async (change, context) => {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (buildRatingDoc.exists) {
                ...
                if (numberOfRatings !== null && numberOfRatings !== undefined && oldRatingTotal !== null && oldRatingTotal !== undefined) {
                    if (change.before.data() !== undefined) {
